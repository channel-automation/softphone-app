...
// Base URL for webhooks - use environment variable or fallback to ngrok URL
const BASE_URL = process.env.WEBHOOK_BASE_URL || 'https://glowing-muskox-sharing.ngrok-free.app';

// Get allowed origins from environment variable or use defaults
const allowedOrigins = process.env.ALLOWED_ORIGINS 
  ? process.env.ALLOWED_ORIGINS.split(',') 
  : ['http://localhost:3039', 'http://localhost:3040', 'http://localhost:3000', 'https://beta.sofphone.channelautomation.com', 'https://glowing-muskox-sharing.ngrok-free.app','https://localhost:7245','https://localhost:6001'];
...
app.get('/api/token', (req, res) => {
  try {
    ...
    const grant = new VoiceGrant({
      ...
      endpointId: `MinimalUIClient:user:${process.env.TWILIO_ACCOUNT_SID}`
    });
...
// New validation endpoint with better error handling
